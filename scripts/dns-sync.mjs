...
const writable = (record) => ({
  ...
  comment: marker,
  ...(['CAA', 'SRV', 'TLSA', 'DS'].includes(record.type)
    ? { data: record.data }
...
async function api(path, method = 'GET', body) {
  if (!zone || !token)
    throw new Error(
      'CLOUDFLARE_ZONE_ID and CLOUDFLARE_DNS_TOKEN are required.',
    );
...
async function current() {
  ...
  let page = 1,
    pages = 1;
  do {
    const result = await api('/dns_records?per_page=500&page=' + page);
...
async function desired() {
  const reserved = JSON.parse(await readFile('registry/reserved.json', 'utf8')),
    records = [];
  for (const file of (await readdir('registry/domains'))
    .filter((file) => file.endsWith('.json'))
    .sort()) {
    const label = file.slice(0, -5),
      definition = validateDefinition(
        label,
        JSON.parse(await readFile('registry/domains/' + file, 'utf8')),
        reserved,
      );
    for (const [type, value] of Object.entries(definition.records)) {
      if (type === 'URL') {
        records.push(
          writable({
            type: 'CNAME',
            name: label + '.lives-on.dev',
            content: 'lives-on.dev',
            ttl: 1,
            proxied: true,
          }),
        );
      ...
      for (const item of Array.isArray(value) ? value : [value]) {
        records.push(
          writable({
            ...
            name: label + '.lives-on.dev',
            ...(type === 'MX'
              ? { content: item.target, priority: item.priority }
              : ['CAA', 'SRV', 'TLSA', 'DS'].includes(type)
                ? { data: item }
                : { content: item }),
          }),
        );
...
function planChanges(live, wanted) {
  if (
    wanted.some(
      (r) => !r.name.endsWith('.lives-on.dev') || r.name.includes('*'),
    )
  )
  ...
  for (const record of wanted)
    if (unmanaged.some((r) => r.name === record.name))
      throw new Error(
        'Unmanaged records already exist at ' +
          record.name +
          '. Review manually.',
      );
  const desiredKeys = new Set(wanted.map(signature)),
    currentKeys = new Set(managed.map(signature));
  const deletes = managed
    .filter((r) => !desiredKeys.has(signature(r)))
    .map((r) => ({ id: r.id }));
  const posts = wanted.filter((r) => !currentKeys.has(signature(r)));
  if (live.length - deletes.length + posts.length > 200)
    throw new Error(
      'The planned change exceeds the configured Free-plan budget of 200 DNS records.',
    );
...
if (mode === 'plan') {
  ...
  if (info.result.name !== 'lives-on.dev')
    throw new Error('Zone is not lives-on.dev.');
  ...
  if (
    ...
    throw new Error('Approval digest or zone mismatch.');
  if (Date.now() - plan.createdAt > 3600000)
    throw new Error('Plan expired; create a fresh diff.');
  const info = await api('');
  if (info.result.name !== 'lives-on.dev' || info.result.status !== 'active')
    throw new Error('The lives-on.dev zone is not active.');
  const live = await current();
  ...
  await writeFile(
    'dns-rollback-plan.json',
    JSON.stringify(plan, null, 2) + '\n',
  );
  console.log('Rollback plan approval SHA256: ' + digest(plan));
} else {
  throw new Error(
    'Use: dns-sync.mjs plan [file] | apply file approvalDigest | rollback-plan backup',
  );
}
